...
const worker = {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const isPublicDocument = request.method === "GET" && (
      ...
      url.pathname.startsWith("/anuncio/") ||
      url.pathname.startsWith("/descubra/") ||
      url.pathname.startsWith("/loja/") ||
      url.pathname === "/lojas" ||
      url.pathname === "/imobiliarias" ||
      url.pathname === "/lojas-de-carros" ||
      url.pathname === "/lojas-virtuais" ||
      url.pathname === "/robots.txt" ||
      url.pathname === "/sitemap.xml" ||
      url.pathname === "/manifest.webmanifest"
    );
    if (isPublicDocument && response.ok) {
